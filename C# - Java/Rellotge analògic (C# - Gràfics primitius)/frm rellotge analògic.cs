...
{
    public partial class frmrellotge_analògic : Form
    {
        ...
        public frmrellotge_analògic()
        {
            InitializeComponent();
            this.MinimumSize = new Size(100, 100);
            SizeChanged += (s, e) => Refresh();
            Paint += paint;
            while (DateTime.Now.Millisecond > 450 && DateTime.Now.Millisecond < 550) { }
            Timer t = new Timer() { Interval = 1000, Enabled = true };
            t.Tick += (s, e) => pintar_hora();
        }
        ...
        void pintar_hora()
        {
            ...
            lbhora.Text = $"{n.Hour.ToString("d2")}:{n.Minute.ToString("d2")}:{n.Second.ToString("d2")}";

            Graphics e = this.CreateGraphics();

            float ah = simplificar(90 - s / 120), am = simplificar(90 - s / 10), aseg = simplificar(90 - n.Second * 6);

            // Despintar línees antigues
