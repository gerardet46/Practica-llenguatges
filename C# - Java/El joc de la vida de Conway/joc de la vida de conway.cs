...
namespace WinForms_CS
{
    ...
    {
        int w = 0, cols = 0, gen = 1;
        bool pintar = false, corrent = false;
        Point actual = new Point(-1, -1);

        List<int> pintat = new List<int>();
        Timer t = new Timer() { Interval = 500 };

        public conway()
        {
            InitializeComponent();
            enregistrar_extensio();

            this.WindowState = FormWindowState.Maximized;
            this.SizeChanged += (s, e) => panel1.Refresh();
            ...
            cbgraella.CheckedChanged += (s, e) => panel1.Refresh();
            panel1.MouseDown += (s, e) =>
            {
                if (e.Button == MouseButtons.Left) pintar = !pintar;
                cel_la(e.X, e.Y);
            };
            panel1.MouseUp += (s, e) => pintar = !pintar;
            panel1.MouseMove += (s, e) => { if (e.Button == MouseButtons.Left) cel_la(e.X, e.Y); };
            panel1.MouseWheel += (s, e) =>
            {
                int delta = 40;
                int d = e.Delta;
                if (amplada.Value + e.Delta / delta <= amplada.Maximum && amplada.Value + e.Delta / delta >= amplada.Minimum)
                {
                    amplada.Value += e.Delta / delta;
                    ample();
                }
            };
            amplada.Scroll += (s, e) => ample();
            velocitat.Scroll += (s, e) =>
            {
                lbvel.Text = $"Velocitat = {velocitat.Value} gen/s";
                if (corrent) t.Stop();
                t.Interval = 1000 / velocitat.Value;
                if (corrent) t.Start();
            };
            ...
            btnborrar.Click += (s, e) => aturar(true);
            btnplay.Click += (s, e) =>
            {
                if (btnplay.Text[1] == 'I')
                {
        ...
        int P2(Point p, int max) => p.X + p.Y * max;

        void ample()
        {
            w = amplada.Value;
            List<Point> p = new List<Point>();
            pintat.ForEach(n => p.Add(P(n, cols)));

            lbample.Text = $"Amplada = {w}px";
            cols = panel1.Width / w + (panel1.Width % w > 0 ? 1 : 0);

            pintat = new List<int>();
            p.ForEach(n => pintat.Add(P2(n, cols) + 1));
        ...
        void cel_la(int x, int y)
        {
            if (!corrent)
            {
                ...
                int p1 = P2(p, cols);
                if (pintat.Contains(p1))
                {
                    pintat.Remove(p1);
                    Point p2 = P(p1, cols);
                    int plus = cbgraella.Checked ? 1 : 0;
                    g.FillRectangle(new SolidBrush(panel1.BackColor), p2.X * w + plus, p2.Y * w + plus, w - plus, w - plus);
                }
                else pintat.Add(p1);
        ...
        void generacio()
        {
            ...
            for (int x = -6; x < cols + 7; x++)
            {
                for (int y = -6; y < rows + 7; y++)
                {
                    ...
                    {
                        if (c == 2 || c == 3) nou.Add(ac);
                    }
                    else if (c == 3) nou.Add(ac);
                }
            }
            Graphics g = panel1.CreateGraphics();
            foreach (int i in nou)
            {
                Point p1 = P(i, cols);
                int plus = cbgraella.Checked ? 1 : 0;
                g.FillRectangle(Brushes.Black, p1.X * w + plus, p1.Y * w + plus, w - plus, w - plus);
            }
            pintat.Where(n => !nou.Contains(n)).ToList().ForEach(i =>
            {
                Point p1 = P(i, cols);
                int plus = cbgraella.Checked ? 1 : 0;
